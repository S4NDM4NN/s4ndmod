...
Name "S4NDMoD PunkBuster Installer"
!define APP_NAME "S4NDMoD - Return to Castle Wolfenstein"
...
${If} $0 == ""
	DetailPrint "Black RtCW NOT found, installing to the default folder!"
	DetailPrint "Patching RtCW 1.0 at $INSTDIR"
	SetOutPath $INSTDIR
	; Clean up old Files
	Delete "$INSTDIR\main\mp_bin.pk3"
	Delete "$INSTDIR\main\mp_pak4.pk3"
	Delete "$INSTDIR\main\mp_pak5.pk3"
	...
	CreateShortCut "$DESKTOP\Return to Castle Wolfenstein S4NDMoD.lnk" "$INSTDIR\Quake3.exe" "+set fs_game s4ndmod"
	
${Else}
	ReadRegStr $1 HKLM "SOFTWARE\Activision\Return to Castle Wolfenstein" "Version"
	${Select} $R1
			${StrRep} $0 $0 'WolfMP.exe,-0' ''
			DetailPrint "Patching RtCW 1.0 at $0"
			SetOutPath $0
			; Clean up old Files
			Delete "$0\main\mp_bin.pk3"
			Delete "$0\main\mp_pak4.pk3"
			Delete "$0\main\mp_pak5.pk3"
			Delete "$0\main\cgame_mp_x86.dll"
			Delete "$0\main\ui_mp_x86.dll"
			Delete "$0\cgame_mp_x86.dll"
			Delete "$0\ui_mp_x86.dll"
			Delete "$0\Quake3.exe"
			Delete "$0\WolfMP.exe"
			Delete "$0\s4ndmod\mp_bin.pk3"
			Delete "$0\s4ndmod\cgame_mp_x86.dll"
			Delete "$0\s4ndmod\ui_mp_x86.dll"
			...
			Delete "$0\pbuser.htm"
			Delete "$0\pbgame.htm"
			CreateShortCut "$DESKTOP\Return to Castle Wolfenstein S4NDMoD.lnk" "$0\Quake3.exe" "+set fs_game s4ndmod"
	${EndSelect}
${EndIf}  
