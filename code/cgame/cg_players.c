...
void CG_AddOnScreenText( const char *text, vec3_t origin, int clientNum ) {
	...
	if ( CG_WorldCoordToScreenCoordFloat( origin, &x, &y ) ) {
		...
		w = MY_Text_Width( text, scale, 0 );
		h = MY_Text_Height( text, scale, 0 );
...
static void CG_PlayerSprites( centity_t *cent ) {
	...
	//S4NDM4NN atm I only care if they are spectators
...
static qboolean CG_PlayerShadow( centity_t *cent, float *shadowPlane ) {
	...
	*shadowPlane = 0;
	...
	// Martin - prevent shadows through wall
...
void CG_Player( centity_t *cent ) {
	...
	vec3_t lightorigin;
	qhandle_t		customShader; // movie_chams
...
//----(SA)	end
	...
	if (movie_chams.integer)
	{
		if(ci->team == TEAM_RED)
			customShader = cgs.media.transGunRed;
		if(ci->team == TEAM_BLUE)
			customShader = cgs.media.transGunBlue;

		legs.customShader=customShader;
		torso.customShader=customShader;
		head.customShader=customShader;
		acc.customShader=customShader;
	}
