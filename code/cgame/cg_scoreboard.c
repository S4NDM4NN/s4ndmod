		...
		if ( atoi( buf ) == -1 ) {
			...
			if ( !cg.latchVictorySound ) {
				...
				if ( cg_announcer.integer ) {
					trap_S_StartLocalSound( trap_S_RegisterSound( "sound/announcer/winallies.wav" ), CHAN_ANNOUNCER );
				}
			}
		} else {
			str = "AXIS";
			...
			if ( !cg.latchVictorySound ) {
				...
				if ( cg_announcer.integer ) {
					trap_S_StartLocalSound( trap_S_RegisterSound( "sound/announcer/winaxis.wav" ), CHAN_ANNOUNCER );
				}
...
static void WM_DrawClientScore( int x, int y, score_t *score, float *color, float fade ) {
	...
	if ( ( score->ping != -1 ) && ( score->ping != 999 ) && ( cg_showFlags.integer ) ) {
		if ( cf_draw( tempx - 7, y - 7, fade, ci->clientNum ) ) { // Modified for rtcw scoreboard
			offset += 14;
			tempx += 18;                // -|| -
			maxchars -= 2;
		}
	...
	if ( ci->team == TEAM_SPECTATOR ) {
		...
		//s = CG_TranslateString( "^3SPECTATOR" );	//Martin - Removed
		translated = CG_TranslateString( "^3SPECTATOR" );
		s = va( "^3(%i) %s", score->ping, translated );
		w = CG_DrawStrlen( s ) * SMALLCHAR_WIDTH;
