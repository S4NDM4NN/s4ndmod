...
//S4NDM4NN - HUD
		healthPos[2] = 3 * ( 20 * scale );
		healthPos[3] = 32 * scale;
//End - HUD edits
...
//S4NDM4NN - HUD
			if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE ) {
				CG_DrawPic( /*lifesPos[0]*/ rect->x + 92, /*lifesPos[1]*/ rect->y - 2, 64, 32, cgs.media.hudAlliedHelmet );
			} else {
				CG_DrawPic( /*lifesPos[0]*/ rect->x + 92, /*lifesPos[1]*/ rect->y - 2, 64, 32, cgs.media.hudAxisHelmet );
			}

			trap_R_SetColor( color );
			CG_DrawField( /*lifesPos[0]+52*/ rect->x + 144, /*lifesPos[1]*/ rect->y, 3, ps->persistant[PERS_RESPAWNS_LEFT], 20 * scale, 32 * scale, qtrue, qtrue );

			//I guess scale can change 0.o
			lifesPos[2] = 64 + ( 20 * scale );
			lifesPos[3] = 32 + ( 32 * scale );
//End - HUD edits
		}
...
//S4NDM4NN - HUD
    //set healthPos
//	rect->x=100;//staminaPos[0];
//	rect->y=100;//staminaPos[1];
	staminaPos[2] = rect->w;
	staminaPos[3] = rect->h;
//End - HUD edits
...
static void CG_DrawWeapRecharge( rectDef_t *rect, vec4_t color, int align ) {
    ...
    //S4NDM4NN - HUD
    //set healthPos
    //rect->x=chargetimePos[0];
    //rect->y=chargetimePos[1];
	chargetimePos[2] = rect->w;
	chargetimePos[3] = rect->h;
//End - HUD edits
