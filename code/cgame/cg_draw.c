...
#define RAO_H 18
#define RAO_W 74
#define RAO_O 225 // offset from team overlay
...
int MY_Text_Width( const char *text, float scale, int limit ) {
	int count,len;
	float out;
	glyphInfo_t *glyph;
	float useScale;
...
int MY_Text_Height( const char *text, float scale, int limit ) {
	int len, count;
	float max;
	glyphInfo_t *glyph;
	float useScale;
...
void MY_Text_Paint( float x, float y, float scale, vec4_t color, const char *text, float adjust, int limit, int style ) {
	int len, count;
	vec4_t newColor;
	glyphInfo_t *glyph;
	float useScale;
...
static float CG_DrawPing( float y ) {
	...
	s = va( "ping %3d", curPing < 999 ? curPing : 999 );
...
static float CG_DrawSpawnTimer( float y ) {
	...
	if ( cgs.clientinfo[cg.snap->ps.clientNum].team == TEAM_RED ) {
		...
		return y; //not on team dont show it
	...
	seconds -= tens * 10;
	if ( mins < 10 ) {
		s = va( "st  %i:%i%i", mins, tens, seconds );
	} else {
		s = va( "st %i:%i%i", mins, tens, seconds );
	}
...
static float CG_DrawSpeedometer( float y ) {
	...
	case 1:         // Miles per hour
		s = va( "%5.1f mph", ( speed / SPEED_US_TO_MPH ) );
		break;
	case 2:         // Kilometers per hour
		s = va( "%5.1f kph", ( speed / SPEED_US_TO_KPH ) );
		break;
...
=====================
*/
static float CG_DrawLagometer( int offset );
...
static void CG_DrawUpperRight( void ) {
	...
	if ( cg_drawSpawnTime.integer ) {
		offset = CG_DrawSpawnTimer( offset );
	}
...
=================
*/
#define NOTIFYLOC_Y 48 // bottom end
#define NOTIFYLOC_X -6
...
static void CG_DrawNotify( void ) {
	...
	if (movie_killMsgs.integer & KILLMSG_REVERSENOTIFYORDER)
		chatHeight = 30;
	else
		chatHeight = cg_popups.integer;
			...
			for(i = 0; i < 30; i++)
			{
				...
				if (cg_widescreen.integer)
				{
					CG_DrawStringExt( NOTIFYLOC_X + TINYCHAR_WIDTH, 
						(5) - (0 - i)*(TINYCHAR_HEIGHT + 3), 
						cgs.notifyMsgs[i % chatHeight], hcolor, qfalse, qfalse,
						((TINYCHAR_WIDTH / 1.77 ) + 3), (TINYCHAR_HEIGHT + 3), 300 );	
				}
				else
				{
					CG_DrawStringExt( NOTIFYLOC_X + TINYCHAR_WIDTH, 
						...
						cgs.notifyMsgs[i % chatHeight], hcolor, qfalse, qfalse,
						TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 300 );
				}
			}
		}
		// Regular Popup Style
			...
			for ( i = cgs.notifyPos - 1; i >= cgs.notifyLastPos; i-- ) {
				...
				if(cg_widescreen.integer)
				{
					CG_DrawStringExt( NOTIFYLOC_X + TINYCHAR_WIDTH,
								  NOTIFYLOC_Y - ( cgs.notifyPos - i ) * TINYCHAR_HEIGHT,
								  cgs.notifyMsgs[i % chatHeight], hcolor, qfalse, qfalse,
								  ((TINYCHAR_WIDTH / 1.77)+1 ), TINYCHAR_HEIGHT+1, maxCharsBeforeOverlay );
				}
				else
				{
					CG_DrawStringExt( NOTIFYLOC_X + TINYCHAR_WIDTH,
...
static float CG_DrawLagometer( int offset ) {
	...
	CG_FillRect( RAO_X,offset,RAO_W,RAO_H * 3,hcolor );
	VectorSet( hcolor, 0.4f, 0.4f, 0.4f );
	CG_DrawRect( RAO_X - 1, offset, RAO_W + 2, RAO_H * 3, 1, hcolor );
...
void CG_CenterPrint( const char *str, int y, int charWidth ) {
	...
	// Martin - Only draw You killed messages (see CG_PriorityCenterPrint())
	if ( movie_cp.integer && strstr( str, "You killed" ) == NULL ) {
		return;
	}
...
void CG_PriorityCenterPrint( const char *str, int y, int charWidth, int priority ) {
	...
	// Martin - Only draw You killed messages (see CG_PriorityCenterPrint())
	// Aparently in recent versions of s4ndmod, You killed now has a priority
	if ( movie_cp.integer && strstr( str, "You killed" ) == NULL ) {
		return;
	}
...
static void CG_DrawCenterString( void ) {
	...
	while ( 1 ) {
		...
		// Martin - Movie
		w = movie_cpSize.integer * CG_DrawStrlen( linebuffer );

		x = ( SCREEN_WIDTH - w ) / 2;

		//CG_DrawStringExt( x, y, linebuffer, color, qfalse, qtrue,	cg.centerPrintCharWidth, (int)(cg.centerPrintCharWidth * 1.5), 0 );
		if ( movie_cpShadow.integer ) {
			CG_DrawStringExt( x, y, linebuffer, color, qfalse, qtrue,
							  movie_cpSize.integer, (int)( movie_cpSize.integer * 1.5 ), 0 );
		} else
		{
...
#define X_POS 30
#define Y_POS 250
void CG_DrawColorTable() {
	int i;
	char line[256];
	int lineNum = 0;
	CG_FillRect( X_POS - 2, Y_POS - 2,rowsize * SMALLCHAR_WIDTH + 4,colsize * SMALLCHAR_HEIGHT + 4,colorWhite );
	CG_DrawRect( X_POS - 2, Y_POS - 2, rowsize * SMALLCHAR_WIDTH + 4, colsize * SMALLCHAR_HEIGHT + 4, 1, colorBlack );
	while ( lineNum < colsize ) {
		line[0] = '\0';
		for ( i = 0; i < rowsize; i++ )
		{
			if ( i + ( lineNum * rowsize ) != '^' ) {
				strcat( line,va( "^%c%c",i + ( lineNum * rowsize ),i + ( lineNum * rowsize ) ) );
			} else {
				strcat( line," " );
			}
		}
		CG_DrawStringExt( X_POS, Y_POS + ( SMALLCHAR_HEIGHT * lineNum ), line, colorWhite,qfalse, qfalse, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 256 );
		lineNum++;
	}
...
static qboolean CG_DrawFollow( void ) {
	...
	if ( movie_playerName.integer)
	{	
		name = cgs.clientinfo[ cg.snap->ps.clientNum ].name;
		CG_DrawStringExt( 10, 450, name, colorWhite, qfalse, qtrue, 12, 12, 0 );
	}
...
static void CG_DrawWarmup( void ) {
	...
	if ( !sec ) {
		if ( cgs.gamestate == GS_WAITING_FOR_PLAYERS ) {
			...
			s2 = CG_TranslateString( "or callvote a start_match" );
	...
	// Martin - prepare to fight
	if ( sec == 10 ) {
		trap_S_StartLocalSound( cgs.media.prepareSound, CHAN_LOCAL_SOUND );

	}
	// Martin
	if ( sec == 5 ) {
		trap_S_StartLocalSound( cgs.media.countdownSound, CHAN_LOCAL_SOUND );
	}
...
void CG_DrawObjectiveIcons() {
	...
	if ( !movie_drawObjectiveIcons.integer)
		return;
...
// jpw
	...
	if ( movie_drawStopwatch.integer ) {
		// draw the stopwatch
		if ( cgs.gametype == GT_WOLF_STOPWATCH ) {
...
//S4NDM4NN - ripped from noquarter(probably ripped from somewhere else)
...
//	int				i;
	...
	for ( clientNum = 0; clientNum < cgs.maxclients; clientNum++ ) {
		...
		MY_Text_Paint( spcNm->x, spcNm->y, spcNm->scale, white, spcNm->text, 0, 0, 0 );
...
static void CG_DrawCompass( void ) {
	//S4NDM4NN - HUD
	float basex = /*compassPos[0];*/ 290;
	float basey = /*compassPos[1];*/ 420;
	float basew = 60;
	float baseh = 60;
	snapshot_t  *snap;
	vec4_t hcolor;
...
static void CG_Draw2D( void ) {
	...
	if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR ) {
		...
		//dont draw crosshair names if they have the name esp on
