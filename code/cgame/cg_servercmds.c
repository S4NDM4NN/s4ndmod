...
// prolly should parse all CS_SERVERINFO keys automagically, but I don't want to break anything that might be improperly set for wolf SP, so I'm just parsing MP relevant stuff here
	...
	//S4NDM4NN - cvars that need to match server side
	trap_Cvar_Set( "g_underwaterRevive",Info_ValueForKey( info,"g_underwaterRevive" ) );
	trap_Cvar_Set( "g_underwaterPistols",Info_ValueForKey( info,"g_underwaterPistols" ) );
	trap_Cvar_Set( "g_underwaterMg",Info_ValueForKey( info,"g_underwaterMg" ) );
	trap_Cvar_Set( "g_jumpAccel",Info_ValueForKey( info,"g_jumpAccel" ) );
	trap_Cvar_Set( "g_deathmatch",Info_ValueForKey( info,"g_deathmatch" ) );
	trap_Cvar_Set( "g_packingPistols",Info_ValueForKey( info,"g_packingPistols" ) );
	trap_Cvar_Set( "g_moonPhysics",Info_ValueForKey( info,"g_moonPhysics" ) );
	trap_Cvar_Set( "g_unlockWeapons",Info_ValueForKey( info,"g_unlockWeapons" ) );
...
//		if(cg_announcer.integer > 0) trap_S_StartLocalSound(cgs.media.countFight, CHAN_ANNOUNCER);
		...
		//CPri("^1FIGHT!\n");
		CG_Printf( "^1FIGHT!\n" );
		CG_PriorityCenterPrint( "^1FIGHT!", SCREEN_HEIGHT * 0.75, BIGCHAR_WIDTH * 0.6, 1 );
...
=======================
isKillMsg
returns true if the print message matches one of the defined flags

=======================
*/
qboolean isKillMsg( const char *str ) {
	qboolean printMsg = qfalse;
	qboolean badMsg = qfalse;
	char lookfor[1024];
	lookfor[0] = '\0';

	if (
		( strstr( str, "gibbed by" ) ) ||
		( strstr( str, "tapped out" ) ) ||
		( strstr( str, "blocked" ) ) ||
		( strstr( str, "revived" ) ) ||
		( strstr( str, "kicked" ) ) ||
		( strstr( str, "banned" ) ) ||
		...
		( strstr( str, "overflow" ) ) ||
		( strstr( str, "dropped" ) ) ||
		( strstr( str, "connected" ) ) ) {
		printMsg = qfalse;
		badMsg = qtrue;
	}

    // Is the player's name in the string?
	if (( strstr( str, cgs.clientinfo[cg.snap->ps.clientNum].name ) != NULL ) && !badMsg  && ( movie_killMsgs.integer & KILLMSG_SHOWPLAYERKILLS )){
		printMsg = qtrue;
	}
/*	else if (( strstr( str, "was" ) != NULL ) && ( movie_killMsgs.integer & KILLMSG_SHOWALLKILLS )){
	{
		printMsg = qtrue;
		
	}
*/
	if ( ( strstr( str, "disconnected" ) != NULL ) && ( movie_killMsgs.integer & KILLMSG_SHOWDISCONNECTS ) ) {
		printMsg = qtrue;
	}

	if ( (( strstr( str, "Player Kicked" ) != NULL ) || ( strstr( str, "kicked player" ) != NULL )) 
		&& ( movie_killMsgs.integer & KILLMSG_SHOWKICKS ) ) {
		printMsg = qtrue;
	}

    // Takes into account single and double digit times. ^32:14 OR ^312:02
	if ( ( movie_killMsgs.integer & KILLMSG_SHOWOBJ ) &&
		 ( str[0] == '^' &&
		   str[1] == '3' &&
		   isdigit( str[2] ) &&
		   ( isdigit( str[3] ) || str[3] == ':' ) ) ) {
...
// Martin - 11/4/09 - Added many more sounds to allow for custom soundpacks
...
#ifdef TEAMSPEAK
	if ( !Q_stricmp( cmd, "testtesttesttest" ) ) {
		connectToTeamspeak( /*CG_Argv(1),atoi(CG_Argv(2)),CG_Argv(3)*/ );
		return;
	}
#endif
