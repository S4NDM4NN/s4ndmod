...
// NERVE - SMF
static void CG_QuickClass_f( void ) {
	//basically a hack to get our info to the ui...
	trap_Cvar_Set( "ui_doquickclass", "1" );
	trap_UI_Popup( "UIMENU_WM_QUICKMESSAGE" );
}
...
static void CG_OpenLimbo_f( void ) {
	...
	if ( currentTeam != mp_team.integer ) {
		trap_Cvar_Set( "mp_team", va( "%d", currentTeam ) );
	}

	if ( currentTeam != mp_currentTeam.integer ) {
...
static void CG_QuickClassMessage_f( void ) {
	int team;
	int playerType;
	int weapon;
	char *teamStr, *classStr, *weapStr;
	char buff[16];
	...
	trap_Cvar_VariableStringBuffer( "mp_playerType", buff, sizeof( buff ) );
	playerType = atoi( buff );
	trap_Cvar_VariableStringBuffer( "mp_weapon", buff, sizeof( buff ) );
	weapon = atoi( buff );
	...
	switch ( weapon )
	{
	case 3: weapStr =   "MP40"; break;
	case 4: weapStr =   "Thompson"; break;
	case 5: weapStr =   "Sten"; break;
	case 6: weapStr =   "Sniper Rifle"; break;
	case 7: weapStr =   "FG42"; break;
	case 8: weapStr =   "Panzerfaust"; break;
	case 9: weapStr =   "Venom"; break;
	case 10: weapStr =  "Flamethrower"; break;
	default: weapStr =  ""; break;
	}
...
static void CG_toggleS4NDui_f( void ) {
	if ( cg_drawS4NDUI.integer ) {
		trap_Cvar_Set( "cg_drawS4NDUI", "0" );
	} else {
		trap_Cvar_Set( "cg_drawS4NDUI", "1" );
...
void CG_autoRecord_f( void ) {
	...
	if ( cg.demoRecording /*|| cl_demorecording.integer*/ ) {
		...
		trap_SendConsoleCommand( va( "g_syncronousClients %i\n", 1 ) );
