...
*/
	...
	for ( ; t <= ent->trailTime ; t += step ) {
		...
		// Martin - 11/12/10 - These are the offsets for the smoke from the can, not direction
...
//----(SA)	modified.  use first person model for finding weapon config name, not third
	if ( item->world_model[W_FP_MODEL] ) {
		...
		if ( !CG_ParseWeaponConfig( va( "%sweapon.cfg", path ), weaponInfo ) ) {
			CG_Printf( "Couldn't register weapon %i (%s) (failed to parse weapon.cfg)", weaponNum, path );
		}
	}
//----(SA)	end
	...
	if ( weaponNum == WP_SHOTGUN ) {
		//register models
		weaponInfo->partModels[W_FP_MODEL][W_PART_1] = trap_R_RegisterModel( "models/weapons2/winchester/v_winchester_slide.md3" );
		weaponInfo->partModels[W_FP_MODEL][W_PART_2] = trap_R_RegisterModel( "models/weapons2/winchester/v_winchester_carrier.md3" );
		weaponInfo->partModels[W_FP_MODEL][W_PART_3] = trap_R_RegisterModel( "models/weapons2/winchester/v_winchester_rarm.md3" );
		weaponInfo->partModels[W_FP_MODEL][W_PART_4] = trap_R_RegisterModel( "models/weapons2/winchester/v_winchester_shell.md3" );
		weaponInfo->partModels[W_FP_MODEL][W_PART_5] = trap_R_RegisterModel( "models/weapons2/winchester/v_winchester_bolt.md3" );
		weaponInfo->partModels[W_FP_MODEL][W_PART_6] = trap_R_RegisterModel( "models/weapons2/winchester/v_winchester_hammer.md3" );
		weaponInfo->partModels[W_FP_MODEL][W_PART_7] = trap_R_RegisterModel( "models/weapons2/winchester/v_winchester_sleeve.md3" );
		weaponInfo->partModels[W_FP_MODEL][W_PART_8] = trap_R_RegisterModel( "models/weapons2/winchester/v_winchester_shell2.md3" );
		weaponInfo->partModels[W_FP_MODEL][W_PART_9] = trap_R_RegisterModel( "models/weapons2/winchester/v_winchester_larm.md3" );
		//register skins
		weaponInfo->shotgunSkins[TEAM_RED][0] = trap_R_RegisterSkin( "models/weapons2/winchester/winchester_rarm_axis.skin" );
		weaponInfo->shotgunSkins[TEAM_BLUE][0] = trap_R_RegisterSkin( "models/weapons2/winchester/winchester_rarm_allied.skin" );
		weaponInfo->shotgunSkins[TEAM_RED][1] = trap_R_RegisterSkin( "models/weapons2/winchester/winchester_slide_axis.skin" );
		weaponInfo->shotgunSkins[TEAM_BLUE][1] = trap_R_RegisterSkin( "models/weapons2/winchester/winchester_slide_allied.skin" );
		weaponInfo->flashModel[W_FP_MODEL] = trap_R_RegisterModel( "models/weapons2/mp40/v_mp40_flash.mdc" );
		weaponInfo->flashModel[W_TP_MODEL] = trap_R_RegisterModel( "models/weapons2/mp40/v_mp40_flash.mdc" );
	}
...
//----(SA)	end
	...
	case WP_SHOTGUN:
		weaponInfo->flashSound[0] = trap_S_RegisterSound( "sound/weapons/shotgun/shotgun_fire.wav" );
		weaponInfo->flashEchoSound[0] = trap_S_RegisterSound( "sound/weapons/shotgun/shotgun_far.wav" );
		weaponInfo->reloadSound = trap_S_RegisterSound( "sound/weapons/shotgun/shotgun_addshell.wav" );
		weaponInfo->lastShotSound[0] = trap_S_RegisterSound( "sound/weapons/shotgun/shotgun_lastshot.wav" );
		weaponInfo->shotgunPump = trap_S_RegisterSound( "sound/weapons/shotgun/shotgun_reloadpump.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		weaponInfo->ammoModel = trap_R_RegisterModel( "models/weapons2/shells/sgshell.md3" );
		break;
...
void CG_RegisterItemVisuals( int itemNum ) {
	...
	wolfkickModel = trap_R_RegisterModel( "models/weapons2/foot/v_wolfoot_10f.mdc" );
...
void CG_AddPlayerWeapon( refEntity_t *parent, playerState_t *ps, centity_t *cent ) {
	...
	if ( drawpart ) {
		...
		if ( isPlayer && !cg.renderingThirdPerson && cg_drawGun.integer >= 2 ) {
			...
			case 6:
				gun.customShader = cgs.media.transGunYellow;
				break;

			case 7:
				gun.customShader = cgs.media.transGunCyan;
				break;

			case 8:
				gun.customShader = cgs.media.transGunMagenta;
				break;
	...
	if ( ps ) {
		...
		for ( i = W_PART_1; i < W_MAX_PARTS; i++ ) {
			...
			if ( weaponNum == WP_SHOTGUN ) {
				if ( i == W_PART_1 ) {
					CG_PositionEntityOnTag( &barrel, parent, "tag_slide", 0, NULL );
				} else if ( i == W_PART_2 )       {
					CG_PositionEntityOnTag( &barrel, parent, "tag_carrier", 0, NULL );
				} else if ( i == W_PART_3 )       {
					CG_PositionEntityOnTag( &barrel, parent, "tag_rarm", 0, NULL );
					if ( ps->persistant[PERS_TEAM] == TEAM_RED ) {
						barrel.customSkin = weapon->shotgunSkins[TEAM_RED][0]; //trap_R_RegisterSkin( "models/weapons2/winchester/winchester_rarm_axis.skin" );
					} else {
						barrel.customSkin = weapon->shotgunSkins[TEAM_BLUE][0]; //trap_R_RegisterSkin( "models/weapons2/winchester/winchester_rarm_allied.skin" );

					}
				} else if ( i == W_PART_4 )       {
					CG_PositionEntityOnTag( &barrel, parent, "tag_shell", 0, NULL );
				} else if ( i == W_PART_5 )       {
					CG_PositionEntityOnTag( &barrel, parent, "tag_bolt", 0, NULL );
				} else if ( i == W_PART_6 )       {
					CG_PositionEntityOnTag( &barrel, parent, "tag_hammer", 0, NULL );
				} else if ( i == W_PART_7 )       {
					CG_PositionEntityOnTag( &barrel, parent, "tag_sleeve", 0, NULL );
				} else if ( i == W_PART_8 )       {
					CG_PositionEntityOnTag( &barrel, parent, "tag_shell2", 0, NULL );
				} else if ( i == W_PART_9 )       {
					CG_PositionEntityOnTag( &barrel, parent, "tag_arm", 0, NULL );
					if ( ps->persistant[PERS_TEAM] == TEAM_RED ) {
						barrel.customSkin = weapon->shotgunSkins[TEAM_RED][1]; //trap_R_RegisterSkin( "models/weapons2/winchester/winchester_slide_axis.skin" );
					} else {
						barrel.customSkin = weapon->shotgunSkins[TEAM_BLUE][1]; //trap_R_RegisterSkin( "models/weapons2/winchester/winchester_slide_allied.skin" );
					}
				}
				...
				if ( drawpart ) {
					...
					if ( isPlayer && !cg.renderingThirdPerson && cg_drawGun.integer >= 2 ) {
						...
						case 6:
							barrel.customShader = cgs.media.transGunYellow;
							break;

						case 7:
							barrel.customShader = cgs.media.transGunCyan;
							break;

						case 8:
							barrel.customShader = cgs.media.transGunMagenta;
							break;
...
//----(SA)	removed
		...
		if ( cg_drawGun.integer >= 2 ) {
			...
			case 6:
				hand.customShader = cgs.media.transGunYellow;
				break;

			case 7:
				hand.customShader = cgs.media.transGunCyan;
				break;

			case 8:
				hand.customShader = cgs.media.transGunMagenta;
				break;
...
// jpw
		break;
	case WP_SHOTGUN: //S4NDM4NN - we want shotgun to leave some marks also
	case WP_LUGER:
	case WP_AKIMBO: //----(SA)	added
	case WP_COLT:
	case WP_MAUSER:
