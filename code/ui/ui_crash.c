...
void crashLog( char *text ) {
	...
	if ( !crashfile ) {
		...
		crashfile = fopen( "S4ND_Crash.log","a+" );
		fputs( va( "log opened: %s \n",asctime( timeinfo ) ),crashfile );
	}
	if ( !crashfile ) {
		return;
	}

	fputs( text,crashfile );
}
...
void CrashHandler( int signal, siginfo_t *siginfo, ucontext_t *ctx ) {
	...
	if ( segvloop < 2 ) {
		...
		crashLog( "   Please forward to S4NDMoD team.   \n" );
		crashLog( "---------------------------------------\n" );
		crashLog( va( "Version: %s Linux\n", Q3_VERSION ) );
		crashLog( va( "Compile Date: %s\n",__DATE__ ) );
