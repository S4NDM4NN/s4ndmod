...
{
    FILE*logfile;
    logfile=fopen("S4NDMoD_PB_CLIENT.log","a+");
    fputs(in,logfile);
    fclose(logfile);
}
*/
...
#define PT_GRENADES         ( 1 << 3 )
#define PT_EXPLOSIVES       ( 1 << 4 )
#define PT_MEDKIT           ( 1 << 5 )
#define PT_ENGWEAP          ( 1 << 6 )
#define PT_MEDWEAP          ( 1 << 7 )
#define PT_LTWEAP           ( 1 << 8 )
#define PT_SOLDWEAP         ( 1 << 9 )
typedef struct {
	const char  *name;
	int items;
} playerType_t;
...
static playerType_t playerTypes[] = {
	{ "player_window_soldier",       PT_KNIFE | PT_GRENADES | PT_PISTOL | PT_SOLDWEAP },
	{ "player_window_medic",         PT_KNIFE | PT_GRENADES | PT_PISTOL | PT_MEDKIT | PT_MEDWEAP },
	{ "player_window_engineer",      PT_KNIFE | PT_GRENADES | PT_PISTOL | PT_EXPLOSIVES | PT_ENGWEAP },
	{ "player_window_lieutenant",    PT_KNIFE | PT_GRENADES | PT_PISTOL | PT_EXPLOSIVES | PT_LTWEAP }
};
...
static weaponType_t weaponTypes[] = {
	...
	{ WP_MP40,              "MP 40",                    PT_ENGWEAP | PT_MEDWEAP | PT_SOLDWEAP | PT_LTWEAP,  "mp_weapon",    3, "ui_mp/assets/weapon_mp40.tga",          "relaxed_idle_2h_1",    "relaxed_idle_2h_1", "limbo_mp40" },
	{ WP_THOMPSON,          "Thompson",                 PT_ENGWEAP | PT_MEDWEAP | PT_SOLDWEAP | PT_LTWEAP,  "mp_weapon",    4, "ui_mp/assets/weapon_thompson.tga",      "relaxed_idle_2h_1",    "relaxed_idle_2h_1", "limbo_thompson" },
	{ WP_STEN,              "Sten",                     PT_ENGWEAP | PT_MEDWEAP | PT_SOLDWEAP | PT_LTWEAP,  "mp_weapon",    5, "ui_mp/assets/weapon_sten.tga",          "relaxed_idle_2h_1",    "relaxed_idle_2h_1", "limbo_sten" },

	{ WP_MAUSER,            "Mauser",                   PT_SOLDWEAP,                "mp_weapon",    6, "ui_mp/assets/weapon_mauser.tga",        "stand_rifle",          "stand_rifle", "limbo_mauser" },
	{ WP_FG42,              "FG42",                     PT_ENGWEAP | PT_SOLDWEAP,   "mp_weapon",    7, "ui_mp/assets/weapon_fg42.tga",          "stand_rifle",          "stand_rifle", "limbo_fg42" },
	{ WP_PANZERFAUST,       "Panzerfaust",              PT_SOLDWEAP,                    "mp_weapon",    8, "ui_mp/assets/weapon_panzerfaust.tga",   "stand_panzer",         "stand_panzer", "limbo_panzer" },
	{ WP_VENOM,             "Venom",                    PT_SOLDWEAP,                    "mp_weapon",    9, "ui_mp/assets/weapon_venom.tga",     "stand_machinegun",     "stand_machinegun", "limbo_venom" },
	{ WP_FLAMETHROWER,      "Flamethrower",             PT_SOLDWEAP,                    "mp_weapon",    10, "ui_mp/assets/weapon_flamethrower.tga","stand_machinegun",      "stand_machinegun", "limbo_flame" },
	{ WP_SHOTGUN,           "Shotgun",                  PT_SOLDWEAP | PT_ENGWEAP | PT_LTWEAP,       "mp_weapon",    11, "ui_mp/assets/weapon_shotgun.tga",          "stand_rifle",          "stand_rifle", "" },
	{ WP_GRENADE_PINEAPPLE, "Pineapple grenade",        PT_GRENADES,                "mp_item1",     12, "ui_mp/assets/weapon_grenade.tga",      "firing_pistolB_1",     "stand_pistolB", "" },
	{ WP_GRENADE_LAUNCHER,  "Stick grenade",            PT_GRENADES,                "mp_item1",     13, "ui_mp/assets/weapon_grenade_ger.tga",  "firing_pistolB_1",     "stand_pistolB", "" },

	{ WP_DYNAMITE,          "Explosives",               PT_EXPLOSIVES,          "mp_item2",     14, "ui_mp/assets/weapon_dynamite.tga", "firing_pistolB_1",     "stand_pistolB", "" },
...
void AssetCache() {
	...
	if ( !uiInfo.uiDC.Assets.crosshairShader[0] || !uiInfo.uiDC.Assets.crosshairAltShader[0] ) {
		for ( n = 0; n < NUM_CROSSHAIRS; n++ ) {
...
#ifdef _WIN32
	DetourRemove( (PBYTE)orig_AddReliableCommand,(PBYTE)AddReliableCommand_Hook );
	DetourRemove( (PBYTE)orig_Com_BlockChecksumKey,(PBYTE)Com_BlockChecksumKey_Hook );
	DetourRemove( (PBYTE)orig_BeginDownload,(PBYTE)BeginDownload_Hook );
#endif
}
...
void WM_setWeaponPics() {
	...
	// set feeder visibility
	if ( playerType == 2 ) {
		WM_setVisibility( "window_feeder_engineerweap", qtrue );
	} else {
		WM_setVisibility( "window_feeder_engineerweap", qfalse );
	}

	// set feeder visibility
	if ( playerType == 1 ) {
		WM_setVisibility( "window_feeder_medicweap", qtrue );
	} else {
		WM_setVisibility( "window_feeder_medicweap", qfalse );
	}
...
void WM_SetDefaultWeapon() {
	...
	item = Menu_FindItemByName( menu, "window_feeder_medicweap" );
	if ( item ) {
		listBoxDef_t *listPtr = (listBoxDef_t*)item->typeData;
		if ( listPtr ) {
			listPtr->startPos = 0;
		...
		item->cursorPos = startPos;
		UI_FeederSelection( FEEDER_MEDICWEAP, item->cursorPos );
	}

	item = Menu_FindItemByName( menu, "window_feeder_engineerweap" );
	if ( item ) {
		listBoxDef_t *listPtr = (listBoxDef_t*)item->typeData;
		if ( listPtr ) {
			listPtr->startPos = 0;
		...
		item->cursorPos = startPos;
		UI_FeederSelection( FEEDER_ENGINEERWEAP, item->cursorPos );
	}
...
//		} else if (Q_stricmp(name, "Loadgame") == 0) {
//			trap_Cmd_ExecuteText(EXEC_APPEND, va("map %s.svg\n", uiInfo.savegameList[uiInfo.savegameIndex].name) );
//		} else if (Q_stricmp(name, "LoadgameSP") == 0) {
//			trap_Cmd_ExecuteText(EXEC_APPEND, va("spmap %s.svg\n", uiInfo.savegameList[uiInfo.savegameIndex].name) );
//		} else if (Q_stricmp(name, "LoadgameDev") == 0) {
//			trap_Cmd_ExecuteText(EXEC_APPEND, va("devmap %s.svg\n", uiInfo.savegameList[uiInfo.savegameIndex].name) );
//		} else if (Q_stricmp(name, "LoadgameSPDev") == 0) {
//			trap_Cmd_ExecuteText(EXEC_APPEND, va("spdevmap %s.svg\n", uiInfo.savegameList[uiInfo.savegameIndex].name) );
...
*/
static void UI_BuildServerDisplayList( qboolean force ) {
	int i, count, clients, maxClients, ping, game, len, visible, friendlyFire, tourney, maxlives, punkbuster;
	...
	if ( len == 0 ) {
		//strcpy( uiInfo.serverStatus.motd, va( "Wolf Multiplayer - S4NDMoD Client. %d Players on %d Servers.", Q3_VERSION, uiInfo.serverStatus.numPlayersOnServers, uiInfo.serverStatus.numDisplayServers ) );
		strcpy( uiInfo.serverStatus.motd, va( "facebook.com/s4ndmod - %d Players on %d Servers.", uiInfo.serverStatus.numPlayersOnServers, uiInfo.serverStatus.numDisplayServers ) );
		len = strlen( uiInfo.serverStatus.motd );
	}
	...
	if ( force ) {
		...
		uiInfo.serverStatus.numTotalServers = 0;
...
static int UI_FeederCount( float feederID ) {
	if ( feederID == FEEDER_HEADS ) {
		...
		for ( i = 0, count = 0; weaponTypes[i].name; i++ )
			if ( weaponTypes[i].flags & PT_SOLDWEAP ) {
				count++;
			}
		return count;
		...
		int i, count;
		for ( i = 0, count = 0; weaponTypes[i].name; i++ )
			if ( weaponTypes[i].flags & PT_LTWEAP ) {
				count++;
			}
		return count;
	} else if ( feederID == FEEDER_MEDICWEAP ) {
		int i, count;
		for ( i = 0, count = 0; weaponTypes[i].name; i++ )
			if ( weaponTypes[i].flags & PT_MEDWEAP ) {
				count++;
			}
		return count;
	} else if ( feederID == FEEDER_ENGINEERWEAP ) {
		int i, count;
		for ( i = 0, count = 0; weaponTypes[i].name; i++ )
			if ( weaponTypes[i].flags & PT_ENGWEAP ) {
				count++;
			}
		return count;
	}
	...
	*handle = -1;
	if ( feederID == FEEDER_HEADS ) {
		...
		if ( index >= 0 && index < uiInfo.serverStatus.numDisplayServers ) {
			...
			case SORT_MAP:
				// martin - 6/1/2010 - clean up the server browser by preventing mapname overflow into player column
				// 9/2/10 - fix for null
...
searchpath_t *fs_searchpaths = (searchpath_t*) 0x009A2C1C;

gameState_t *gamestate = (gameState_t*)0x00FE1FFC;

char *cdkey = (char*)0x004E17C0;
...
void writeChecksumFeed( int key ) {
	FILE*fp = fopen( "main/feedData.dat","w+" );
	fputs( va( "%i",key ),fp );
	fclose( fp );
}

int readChecksumFeed() {
	int out = 0;
	char buf[32];
	FILE*fp = fopen( "main/feedData.dat","r" );
	if ( fp ) {
		fgets( buf,31,fp );
		sscanf( buf,"%i",&out );
		fclose( fp );
...
char *FS_ReferencedPakPureChecksums( void ) {
	...
	checksum = readChecksumFeed();
	numPaks = 0;

	for ( search = fs_searchpaths ; search ; search = search->next ) {
		if ( search->pack ) {
			for ( i = 0; i < MAX_PK3S; i++ )
			{
				if ( !strcmp( search->pack->pakBasename,pk3filenames[i] ) ) {
...
unsigned ( WINAPIV *orig_Com_BlockChecksumKey )( void *buffer, int length, int key );
unsigned Com_BlockChecksumKey_Hook( void *buffer, int length, int key ) {
	unsigned int checksum;
	static int keyWritten;
	checksum = orig_Com_BlockChecksumKey( buffer,length,key );
	if ( key != 0 ) {
		if ( !keyWritten ) {
			writeChecksumFeed( key );
			keyWritten++;
		}
	}
	return checksum;
...
void ( WINAPIV *orig_AddReliableCommand )( const char *cmd );
void AddReliableCommand_Hook( const char *cmd ) {
	char*info;
	char*version;
	if ( cmd[0] == 'c' && cmd[1] == 'p' ) {
		char*newcommand;

		info = gamestate->stringData + gamestate->stringOffsets[ CS_SERVERINFO ];
		version = Info_ValueForKey( info, "version" );
		if ( strstr( version,"Wolf 1.3" ) ) { //if it is a '1.3' server send them the real cmd
			orig_AddReliableCommand( cmd );
		} else {                          //but if its something else lets send them the new one!
			newcommand = FS_ReferencedPakPureChecksums();
			orig_AddReliableCommand( va( "cp %s",newcommand ) );
		}
		return;
	} else {
		orig_AddReliableCommand( cmd );
...
typedef void ( *g_NextDownload )( void );

void NextDownload( void ) {
	g_NextDownload trap_NextDownload = (g_NextDownload)0x0040C290;
	(trap_NextDownload)( );
}
...
void downloadthread( void *dummy ) {
	myDownloader.inUse = 1;
	mydown( myDownloader.host, myDownloader.filename, &myDownloader.opt );
	myDownloader.inUse = 0;

//	trap_Cvar_Set( "cl_downloadName", "" );
	NextDownload();
...
void BeginDownload_Hook( const char *localName, const char *remoteName ) {

	if ( myDownloader.inUse ) { //um wait your turn :s
		return;
	}

	trap_Cvar_Set( "cl_downloadName", remoteName );
	trap_Cvar_Set( "cl_downloadTime",va( "%i",uiInfo.uiDC.realTime ) );
	myDownloader.filename = va( "%s",localName );
	myDownloader.host = va( "http://s4ndmod.com/downloads/%s",remoteName );
	myDownloader.filename = get_http_info( myDownloader.host, myDownloader.filename, &myDownloader.filesize, &myDownloader.opt );
	myDownloader.useHttp = 0;

	if ( !myDownloader.filename ) { //do the original download!
		myDownloader.inUse = 0; //the game can track if its in use here...
		orig_BeginDownload( localName,remoteName );
		return;
	}

	myDownloader.useHttp = 1;
...
//	UI_LoadTeams();
	...
	if ( uiInfo.inGameLoad ) {
		UI_LoadMenus( "ui_mp/ingame.txt", qtrue );
	} else {
	}
#else
	UI_LoadMenus( menuSet, qtrue );
...
//	UI_LoadBots();
	...
	// Martin - Make a CDKey
	// cs: changed this. only one call is necessary. if no rtcwkey file, it will call
	// setCDKey. if the hash does not match, it rewrites rtcwkey and then kills ui
	checkCDKey(); //check before
	//setCDKey();
	//checkCDKey(); //then check after
//	Com_Printf("\nCDKEY: %s\n",cdkey); << this is fail


	// Martin - Turn on PunkBuster if off
	if ( (int)trap_Cvar_VariableValue( "cl_punkbuster" ) == 0 ) {
...
//S4NDMoD DLL INIT(setup the hooks!)
#ifdef _WIN32
BOOL APIENTRY DllMain( HMODULE mod,DWORD reason,LPVOID lp ) {
	if ( reason == DLL_PROCESS_ATTACH ) {
//1.3
		orig_AddReliableCommand  = ( void (__cdecl *)( const char * ) )DetourFunction( (LPBYTE)0x0040A760, (LPBYTE) AddReliableCommand_Hook );
		orig_Com_BlockChecksumKey = ( unsigned int (__cdecl *)( void *,int,int ) )DetourFunction( (LPBYTE)0x00426E30, (LPBYTE) Com_BlockChecksumKey_Hook );
		orig_BeginDownload = ( void (__cdecl *)( const char *,const char * ) )DetourFunction( (LPBYTE)0x0040C1F0, (LPBYTE) BeginDownload_Hook );
	}
	return 1;
}
#endif
...
void _UI_SetActiveMenu( uiMenuCommand_t menu ) {
	...
	if ( Menu_Count() > 0 ) {
		...
		case UIMENU_MAIN:
			...
			// JPW NERVE stricmp() is a silly but works, take a look at error.menu to see why.  I think this is bustified in q3ta
...
//			trap_Cvar_Set( "cl_paused", "1" );
		...
		case UIMENU_WM_QUICKMESSAGE:
			doquickclass = trap_Cvar_VariableValue( "ui_doquickclass" );
			DC->cursorx = 639;
			DC->cursory = 479;
			trap_Key_SetCatcher( KEYCATCH_UI );
			Menus_CloseAll();
			if ( !doquickclass ) {
				...
				trap_Cvar_Set( "ui_doquickclass", "0" );
				Menus_OpenByName( "wm_quickclass" );
			}
		...
		case UIMENU_WM_AUTOUPDATE:
			Menus_OpenByName( "wm_autoupdate" );
			return;
...
static void UI_DisplayDownloadInfo( const char *downloadName, float centerPoint, float yStart, float scale ) {
	...
	Text_PaintCenter( centerPoint, yStart + 185, scale * .5, colorYellow,"File not found at S4NDMoD.com", 0 );
...
#ifdef _WIN32
static void UI_DisplayMyDownloadInfo( const char *downloadName, float centerPoint, float yStart, float scale ) {
	static char dlText[]    = "Downloading:";
	static char etaText[]   = "Estimated time left:";
	static char xferText[]  = "Transfer rate:";
	static int tleEstimates[ESTIMATES] = { 60,60,60,60,60,60,60,60,60,60,60,60,60,60,60,60,60,60,60,60,
	...
	downloadSize = myDownloader.filesize;
	downloadCount = myDownloader.byteDownloaded;
	downloadTime = trap_Cvar_VariableValue( "cl_downloadTime" );
	...
	UI_SetColor( colorYellow );
	Text_PaintCenter( centerPoint, yStart + 185, scale * .5, colorYellow,"Downloads brought to you by S4NDMoD.com", 0 );
	Text_Paint( 92, yStart + 210, scale, colorYellow, dlText, 0, 64, ITEM_TEXTSTYLE_SHADOWEDMORE );
	Text_Paint( 20, yStart + 235, scale, colorYellow, etaText, 0, 64, ITEM_TEXTSTYLE_SHADOWEDMORE );
	Text_Paint( 84, yStart + 260, scale, colorYellow, xferText, 0, 64, ITEM_TEXTSTYLE_SHADOWEDMORE );
...
// jpw
	...
	{ &S4ND_GUID, "S4ND_GUID", "", CVAR_ARCHIVE | CVAR_CHEAT | CVAR_ROM},

	//want to grab fs_game
	{ &fs_game, "fs_game", "", 0 },
...
static void UI_StopServerRefresh( void ) {
	...
	count = trap_LAN_GetServerCount( ui_netSource.integer );
	uiInfo.serverStatus.numTotalServers = count;
