...
#ifdef GLOBALSTATS
			stats_event_objectiveReturned( other );
#endif
...
#ifdef GLOBALSTATS
		stats_event_objectiveStolen( other );
#endif
		//S4NDM4NN-spy
...
#ifdef GLOBALSTATS
	stats_event_checkpointCaptured( other );
#endif
// jpw
...
void checkpoint_spawntouch( gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	if ( self->s.frame == WCP_ANIM_NOFLAG ) {
		...
		// Martin - 12/12/10 - No flag reclaim is retarded.
		AddScore( other, WOLF_SP_RECOVER );
#ifdef GLOBALSTATS
		stats_event_spawnflagReclaimed( other );
#endif
	}
