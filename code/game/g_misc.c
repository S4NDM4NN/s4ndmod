...
void TeleportPlayer( gentity_t *player, vec3_t origin, vec3_t angles ) {
	...
	//ANTILAG - reset the origin trail for this player to avoid lerping in a time shift
	G_ResetTrail( player );
}
...
void mg42_muzzleflash( gentity_t *ent ) {
	...
	if ( flash ) {
		VectorCopy( ent->s.apos.trBase, flash->s.apos.trBase );

		VectorCopy( ent->s.pos.trBase, point );
		AngleVectors( flash->s.apos.trBase, forward, NULL, NULL );
		VectorMA( point, 40, forward, point );

		SnapVector( point );
		G_SetOrigin( flash, point );
		SnapVector( flash->s.apos.trBase );
...
#ifdef GLOBALSTATS  //let stats know we are shooting the mg42!
	if ( ent && ent->client ) {
		stats_event_shotFired( ent, 0 );
	}
#endif
}
...
void mg42_track( gentity_t *self, gentity_t *other ) {
	...
	if ( other->active ) {
		// DHM - Nerve :: Set firing rate based on time
		if ( other->client && ( other->client->buttons & BUTTON_ATTACK )
			 && ( ( level.time - self->last_move_time ) >= MG42_RATE_OF_FIRE ) ) {
...
//				VectorMA (muzzle, 16, forward, muzzle); // JPW NERVE unnecessary and makes it so close-range enemies get missed
		...
		} else if ( other->client && ( other->client->buttons & BUTTON_ATTACK ) && !( self->s.frame ) )         {
...
void mg42_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod ) {
	...
	if ( gun && self->health <= 0 ) {
		...
		if ( g_gametype.integer >= GT_WOLF ) {
			...
			self->health = 0;
			G_AddEvent( self, EV_GENERAL_SOUND, G_SoundIndex( "sound/world/metalbreak.wav" ) );
			G_AddEvent( self, EV_GENERAL_SOUND, G_SoundIndex( "sound/world/metalbreak.wav" ) );
		}
		// dhm - end
	}
