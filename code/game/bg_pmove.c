...
static qboolean PM_CheckJump( void ) {
	...
	int jumpAccel = g_jumpAccel.integer;
	int moonPhysics = g_moonPhysics.integer;
	#elif defined( CGAMEDLL )
	extern vmCvar_t cg_jumpAccel;
	extern vmCvar_t cg_moonPhysics;
	int jumpAccel = cg_jumpAccel.integer;
	int moonPhysics = cg_moonPhysics.integer;
	#else
	int jumpAccel = 0;
	int moonPhysics = 0;
	#endif
...
static void PM_BeginShotgunReload( void ) {
	...
	BG_AnimScriptEvent( pm->ps, ANIM_ET_RELOAD_SG1, qfalse, qtrue );

	if ( pm->ps->ammoclip[BG_FindClipForWeapon( WP_SHOTGUN )] == 0 ) {
	...
	pm->ps->weaponTime += animTime /*BG_Weapons[pm->ps->weapon].animationTime[anim]*/;
...
static void PM_ContinueShotgunReload( void ) {
	...
	BG_AnimScriptEvent( pm->ps, ANIM_ET_RELOAD_SG2, qfalse, qtrue );

	PM_StartWeaponAnim( WEAP_RELOAD2 );

	pm->ps->weaponTime += 1000 / 32 * 22 /*BG_Weapons[pm->ps->weapon].animationTime[WEAP_RELOAD2]*/;
...
static void PM_Shotgun_PumpToLoop( void ) {
	...
	pm->ps->weaponTime += 1000 / 26 * 5 /*BG_Weapons[pm->ps->weapon].animationTime[WEAP_ALTSWITCHTO]*/;
...
static void PM_StopShotgunReload( void ) {
	...
	BG_AnimScriptEvent( pm->ps, ANIM_ET_RELOAD_SG3, qfalse, qtrue );

	PM_StartWeaponAnim( WEAP_RELOAD3 );
	pm->ps->weaponTime += 1000 / 26 * 11 /*BG_Weapons[pm->ps->weapon].animationTime[WEAP_RELOAD3]*/;
...
static void PM_Shotgun_StopFromPump( void ) {
	...
	pm->ps->weaponTime += 1000 / 20 * 6 /*BG_Weapons[pm->ps->weapon].animationTime[WEAP_DROP2]*/;
	pm->ps->holdable[0] = SHOTGUN_RELOADING_STOP;
}
...
static void PM_BeginWeaponReload( int weapon ) {
	qboolean shotgunReload = ( weapon == WP_SHOTGUN && pm->ps->holdable[0] != SHOTGUN_NOT_RELOADING );
	...
	if ( ( weapon < WP_BEGINGERMAN || weapon > WP_DYNAMITE2 ) && weapon != WP_SHOTGUN ) {
		return;
	}
...
qboolean isAutoreloadWeapon( int weapon ) {
	switch ( weapon ) {
	case WP_MP40:
	case WP_THOMPSON:
	case WP_STEN:
	...
	return qfalse;  // all other guns :D
}
...
void PM_CheckForReload( int weapon ) {
	...
	qboolean autoReload;        //Autoreload
...
#if defined ( GAMEDLL )
	autoReload = pm->ps->autoReload;
#elif defined ( CGAMEDLL )
	autoReload = cg_autoReload.integer;
#else
	autoReload = 0;
#endif
...
//MARTIN _ BELOW IS OLD INCASE MESSUP
	...
	//testing..works 3/15/08
	else if ( !( isAutoreloadWeapon( pm->ps->weapon ) ) && !( pm->ps->ammoclip[BG_FindClipForWeapon( weapon )] ) &&
			  pm->ps->ammo[BG_FindAmmoForWeapon( weapon )] ) {
		PM_BeginWeaponReload( weapon );
	}
// END MARTIN
...
// jpw
		...
		if ( pm->ps->weapon == WP_SNIPERRIFLE || pm->ps->weapon == WP_SNOOPERSCOPE || pm->ps->weapon == WP_FG42SCOPE ) {
			for ( i = 0; i < 2; i++ )
...
//S4NDM4NN since g_local isnt included in this one
	...
	if ( ( pm->ps->isSpy ) && ( pm->ps->weapon != WP_KNIFE && pm->ps->weapon != WP_KNIFE2 && pm->ps->weapon != WP_SNIPERRIFLE && pm->ps->weapon != WP_SMOKE_GRENADE ) ) {
		if ( pm->cmd.buttons & BUTTON_ATTACK ) {
			if ( ( pm->ps->eFlags & EF_ZOOMING ) && ( pm->ps->stats[STAT_KEYS] & ( 1 << INV_BINOCS ) ) ) {
				...
				trap_SendServerCommand( pm->ps->clientNum, "cp \"^3You have lost your disguise!\"" );
...
// jpw
	...
	if ( pm->ps->weaponstate == WEAPON_RELOADING ) {
		if ( pm->ps->weapon == WP_SHOTGUN ) {
			if ( pm->ps->holdable[0] == SHOTGUN_RELOADING_BEGIN_1ST ||
				 ( pm->ps->holdable[0] == SHOTGUN_RELOADING_INTERRUPT && pm->ps->ammoclip[BG_FindClipForWeapon( WP_SHOTGUN )] == 0 ) ) {
				pm->ps->ammoclip[BG_FindClipForWeapon( WP_SHOTGUN )] += 1;
				pm->ps->ammo[BG_FindAmmoForWeapon( WP_SHOTGUN )] -= 1;

				if ( pm->ps->holdable[0] == SHOTGUN_RELOADING_INTERRUPT || pm->ps->ammo[BG_FindAmmoForWeapon( WP_SHOTGUN )] == 0 ) {
			...
			if ( pm->ps->holdable[0] == SHOTGUN_RELOADING_LOOP || pm->ps->holdable[0] == SHOTGUN_RELOADING_INTERRUPT ) {
				...
				ammo = pm->ps->ammoclip[BG_FindClipForWeapon( WP_SHOTGUN )];
				ammo += 1;

				if ( ammo > 6 || pm->ps->ammo[BG_FindAmmoForWeapon( WP_SHOTGUN )] == 0 ) {
					PM_StopShotgunReload();
					return;
				} else {
					pm->ps->ammoclip[BG_FindClipForWeapon( WP_SHOTGUN )] += 1; // add one shell into shotgun
					pm->ps->ammo[BG_FindAmmoForWeapon( WP_SHOTGUN )] -= 1; // grab one from ammo

					if ( pm->ps->holdable[0] == SHOTGUN_RELOADING_INTERRUPT ||  ammo == 6 || pm->ps->ammo[BG_FindAmmoForWeapon( WP_SHOTGUN )] == 0 ) {
						PM_StopShotgunReload();
					} else {
						PM_ContinueShotgunReload();
					}
	...
	if ( pm->gametype >= GT_WOLF
		...
		if ( pm->ps->weapon == WP_SMOKE_GRENADE ) {
			...
			if ( ( g_smokeGrenades.integer ) && ( pm->ps->throwSmoke ) ) {
				if ( pm->cmd.serverTime - pm->ps->classWeaponTime < ( pm->ltChargeTime * 0.25f ) ) {
					return;
				}
			} else
	...
	if ( pm->waterlevel == 3 ) {
		...
		int underwaterRevive = g_underwaterRevive.integer;
		int underwaterPistols = g_underwaterPistols.integer;
		int underwaterMg = g_underwaterMg.integer;
	#elif defined( CGAMEDLL )
		extern vmCvar_t cg_underwaterRevive;
		extern vmCvar_t cg_underwaterPistols;
		extern vmCvar_t cg_underwaterMg;
		int underwaterRevive = cg_underwaterRevive.integer;
		int underwaterPistols = cg_underwaterPistols.integer;
		int underwaterMg = cg_underwaterMg.integer;
	#else
		int underwaterRevive = 0;
		int underwaterPistols = 0;
		int underwaterMg = 0;
	#endif
		qboolean cantfire = qtrue; //you cant fire untill proven otherwise
		//default weapons that CAN fire underwater
		// Martin - 1/1/2012 - Updated allowed default under water weapons(pliers, ammo, medkit)
		if ( pm->ps->weapon == WP_KNIFE ||
			 ...
			 pm->ps->weapon == WP_DYNAMITE ||
			 pm->ps->weapon == WP_DYNAMITE2 ) {
			cantfire = qfalse; //they can fire with these
		}

		//let guns fire underwater
		// Martin - 1/1/2012 - Added missing weapons (smoke can, binocs)
		if ( underwaterMg ) {
			if ( pm->ps->weapon == WP_MP40 ||
				 pm->ps->weapon == WP_THOMPSON ||
				 pm->ps->weapon == WP_STEN ||
				 pm->ps->weapon == WP_MAUSER ||
				 pm->ps->weapon == WP_SNIPERRIFLE ||
				 pm->ps->weapon == WP_FG42 ||
				 pm->ps->weapon == WP_FG42SCOPE ||
				 pm->ps->weapon == WP_VENOM ||
				 pm->ps->weapon == WP_FLAMETHROWER ||
				 pm->ps->weapon == WP_PANZERFAUST ||
				 pm->ps->weapon == WP_BINOCULARS ||
				 pm->ps->weapon == WP_SMOKE_GRENADE ||
				 pm->ps->weapon == WP_SHOTGUN ) {
				cantfire = qfalse;
			}
		}

		if ( underwaterPistols ) {
			if ( pm->ps->weapon == WP_LUGER ||
		...
		if ( underwaterRevive ) {
			if ( pm->ps->weapon == WP_MEDIC_SYRINGE ) {
				cantfire = qfalse;
			}
		}
		...
		if ( cantfire ) {
			...
			pm->ps->weaponTime  = 500;
			return; //since they cant fire return here
		}
...
//MARTIN=================================
			...
			//if ((pm->ps->autoReload == qfalse) && (IS_AUTORELOAD_WEAPON(pm->ps->weapon)) && !(pm->cmd.wbuttons & WBUTTON_RELOAD)){
			if ( ( autoReload == qfalse ) && ( isAutoreloadWeapon( pm->ps->weapon ) ) && !( pm->cmd.wbuttons & WBUTTON_RELOAD ) ) {
				reloading = qfalse;
			}
			// Martin - explanation...This can be 1 of 2 options
			// 1. Autoreload off, but it is a gun that should reload itself
			// 2. Autoreload on, so all guns will reload themselves
			//3/15/08 - completly reworked it...check pm_CHECKFORRELOAD for addational code
// end martin============================
...
void PM_UpdateLean( playerState_t *ps, usercmd_t *cmd, pmove_t *tpm ) {
	...
	if ( leaning ) {
		/*	VectorCopy( ps->origin, start );
		    ...
		    VectorCopy( ps->viewangles, viewangles );	//agent removed
		    viewangles[ROLL] = 0;
		...
		*/
		...
		VectorSet( tmins, -8, -8, -7 ); // ATVI Wolfenstein Misc #472, bumped from -4 to cover gun clipping issue(was -7)
		VectorSet( tmaxs, 8, 8, 4 );
//END DUMP----------------------------
...
void PM_Sprint( void ) {
	if ( pm->cmd.buttons & BUTTON_SPRINT && ( pm->cmd.forwardmove || pm->cmd.rightmove ) && !( pm->ps->pm_flags & PMF_DUCKED ) ) {
		...
		if ( pm->ps->powerups[PW_NOFATIGUE] ) { // (SA) recharge at 2x with stamina powerup
			...
			if ( pm->gametype != GT_SINGLE_PLAYER ) {
				// L0 - fixed the crouch bug where it didn't recharge stamina when crouching if cvar was disabled...
				extern vmCvar_t g_crouch;
				if ( g_crouch.integer & CROUCH_FASTSTAMINA ) {
					if ( pm->ps->pm_flags & PMF_DUCKED ) {  // L0- moved this under cvar
						pm->ps->sprintTime += 4000 * pml.frametime;   // 4000 is a good # for g_Test1 takes about 5 secs crouched
					}
					pm->ps->sprintTime += 500 * pml.frametime;    // L0 - we still need this for when we are standing
...
void PmoveSingle( pmove_t *pmove ) {
	...
	if ( pm->cmd.wbuttons & WBUTTON_ZOOM && pm->ps->stats[STAT_HEALTH] >= 0 ) {
		if ( pm->ps->stats[STAT_KEYS] & ( 1 << INV_BINOCS ) ) {        // (SA) binoculars are an inventory item (inventory==keys)
			if ( pm->ps->weapon != WP_SNIPERRIFLE && pm->ps->weapon != WP_SNOOPERSCOPE && pm->ps->weapon != WP_FG42SCOPE ) {    // don't allow binocs if using the sniper scope
	...
	*/
	...
	if ( !isDummy ) {
		...
		// 3/27/2010 - Only run fixed physics for living players (so corpses don't slide for miles down hills)
