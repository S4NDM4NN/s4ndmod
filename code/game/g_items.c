...
int Pickup_Key( gentity_t *ent, gentity_t *other ) {
	...
	if ( ( g_deathmatch.integer != 2 ) && ( g_binocWar.integer ) && ( !g_sniperWar.integer ) && ( !g_goldrush.integer ) ) {
		if ( !Q_stricmp( ent->item->classname, "key_binocs" ) ) {
			other->client->pers.numBinocs++;
		}
	}
	if ( g_gametype.integer == GT_SINGLE_PLAYER ) {
		return RESPAWN_SP;
...
/* OLD way
    if( ammoweap == WP_GRENADE_LAUNCHER || ammoweap==WP_GRENADE_PINAPPLE ) {			// make sure if he picks up a grenade that he get's the "launcher" too
        COM_BitSet( ent->client->ps.weapons, WP_GRENADE_LAUNCHER );
...
#ifdef GLOBALSTATS //let stats know we picked up a nadepack
	if ( !Q_stricmp( ent->item->classname,"ammo_grenades" ) || !Q_stricmp( ent->item->classname,"ammo_grenades_american" ) ) {
		stats_event_pickupPack( other,ent->parent,STATS_NADEPACK );
	}
#endif
...
#ifdef GLOBALSTATS //let stats know ammo was picked up
		stats_event_pickupPack( other,ent->parent,STATS_AMMO );
#endif
...
// if LT isn't giving ammo to self or another LT or the enemy, give him some props
		...
		if ( ( ent->parent ) && ( ent->parent != other ) && ( OnSameTeam( ent->parent, other ) ) ) {
			...
			if ( g_gamestate.integer == GS_PLAYING ) {
				if ( ent->parent->client->pers.ammoPacks > level.HAammonum ) {
					Q_strncpyz( level.HAhighammogivers, "", sizeof( level.HAhighammogivers ) );
					strcat( level.HAhighammogivers, ent->parent->client->pers.netname );
					level.HAammonum = ent->parent->client->pers.ammoPacks;
				} else if ( ent->parent->client->pers.ammoPacks == level.HAammonum )     {
					strcat( level.HAhighammogivers, "^7, " );  //Add a comma after the other person
					strcat( level.HAhighammogivers, ent->parent->client->pers.netname );  //and add their name
				}
			}
...
// everybody likes grenades -- abuse weapon var as grenade type and i as max # grenades class can carry
		if ( g_deathmatch.integer != 2 ) {
			...
			case PC_MEDIC:
				...
				break;
			case PC_SOLDIER:
				i = g_soldNades.integer;
				break;
			case PC_ENGINEER:
				i = g_engNades.integer;
				break;
			default:
				i = 1;
				break;
		...
		//S4NDM4NN-a little bit extra stuff in ammo packs
		...
		if ( ( other->client->pers.throwingKnives < g_maxKnives.integer ) || ( g_maxKnives.integer == 0 ) ) {
			if ( g_ammoGivesKnife.integer ) {
		...
		//S4NDM4NN - New code for adding ammo to pistols(for g_packingPistols)
		if ( COM_BitCheck( other->client->ps.weapons, WP_LUGER ) ) { //if they have ammo add it
		...
		if ( COM_BitCheck( other->client->ps.weapons, WP_COLT ) ) { //if they have ammo add it
...
// jpw
	...
	} else if ( ent->item->giTag == WP_FG42 ) {
...
#ifdef GLOBALSTATS //let stats know a medkit was picked up
	stats_event_pickupPack( other,ent->parent,STATS_MEDKIT );
#endif
...
// if medic isn't giving ammo to self or another medic or the enemy, give him some props
	...
	if ( ( ent->parent ) && ( ent->parent != other ) && ( OnSameTeam( ent->parent, other ) ) ) {
		...
		if ( g_gamestate.integer == GS_PLAYING ) {
			if ( ent->parent->client->pers.medPacks > level.HAmednum ) {
				Q_strncpyz( level.HAhighmedgivers, "", sizeof( level.HAhighmedgivers ) );
				strcat( level.HAhighmedgivers, ent->parent->client->pers.netname );
				level.HAmednum = ent->parent->client->pers.medPacks;
			} else if ( ent->parent->client->pers.medPacks == level.HAmednum )     {
				strcat( level.HAhighmedgivers, "^7, " );  //Add a comma after the other person
				strcat( level.HAhighmedgivers, ent->parent->client->pers.netname );  //and add their name
			}
		}
	}
...
void Touch_Item( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	//S4NDM4NN-check ammo
	if ( ( ent->item->giType == IT_WEAPON ) && ( ent->item->giTag == WP_AMMO ) && ( !CheckAmmo( other ) ) ) {
		return;
	}
	//End
	...
	case IT_KEY:
		...
		break;
	case IT_TREASURE:
		other->client->pers.gold++;
		trap_SendServerCommand( other - g_entities, va( "cp \"^3You have ^5%i^3 Gold!", other->client->pers.gold - 1 ) );
		if ( other->client->pers.gold - 1 == 5 || other->client->pers.gold - 1 == 10 ) {
			G_TeamCommand( other->client->sess.sessionTeam, va( "chat \"%s^7: ^3Bring me your gold I have ^5%i ^3already!\"", other->client->pers.netname, other->client->pers.gold - 1 ) );
		}
...
gentity_t *LaunchItem( gitem_t *item, vec3_t origin, vec3_t velocity, int ownerNum ) {
	...
	if ( item->giType == IT_TEAM ) { // Special case for CTF flags
		...
		if ( !g_makeDMspawns.integer ) {
			...
			dropped->nextthink = level.time + 1000000000;
		}
	}
...
//			but for now, re-register the MP40 automatically
	//RegisterItem( BG_FindItemForWeapon( WP_BINOCULARS ) );
