...
static int _weaponBotToGame( int weapon ) {
	...
	case RTCW_WP_FG42:
		return WP_FG42;
	case RTCW_WP_FG42SCOPE:
		return WP_FG42SCOPE;
	case RTCW_WP_SHOTGUN:
		return WP_SHOTGUN;
	default:
		return WP_NONE;
	}
...
int Bot_WeaponGameToBot( int weapon ) {
	...
	case WP_FG42:
		return RTCW_WP_FG42;
	case WP_FG42SCOPE:
		return RTCW_WP_FG42SCOPE;
	case WP_SHOTGUN:
		return RTCW_WP_SHOTGUN;
	default:
		return RTCW_WP_NONE;
	}
}
...
static int _GetEntityClass( gentity_t *_ent ) {
	...
	case ET_MISSILE:
	{
		...
		case WP_SMOKE_GRENADE:
			// only the a/s cannister ...
			if ( !(_ent->s.eFlags & EF_SMOKINGBLACK) ) {
...
int AddBot( const MessageHelper &_data ) {
	if ( level.framenum < 6 ) { // no GAME_INIT_FRAMES in 1.0
		return -1;
	}
	...
	guid << "OMNIBOT" << std::setw( 2 ) << std::setfill( '0' ) << std::right << std::setw( 25 ) << "";
