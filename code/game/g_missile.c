...
void G_MissileImpact( gentity_t *ent, trace_t *trace, int impactDamage ) {
	...
	if ( !other->takedamage &&
		...
		return;
	}   else if ( !other->takedamage && ( ent->s.eFlags & EF_STICK ) )   {
	...
	if ( other->takedamage ) {
		...
		} else    // if no damage value, then this is a splash damage grenade only
		{
			...
			if ( ( ent->s.eFlags & EF_STICK ) && ( !other->client ) ) {
				G_SetOrigin( ent, trace->endpos );    //'stick' to it
			} else {
...
//layer = AICast_FindEntityForName( "player" );
		player = NULL;
		if ( !player ) {
			return;
		}
...
//==========================================
//BEGIN modified G_ExplodeMissile,

/*
Dutchmeat:
Don't forget to add this in the g_local header file, in the gentity struct:
...
// jpw
	...
	if ( etype == ET_MISSILE ) {
		...
		if ( ent->s.weapon == WP_DYNAMITE && ent->methodOfDeath != MOD_LANDMINE ) { // do some scoring
...
//				// shot heard round the world...
                    gentity_t *player;
    player = NULL;					//			player = AICast_FindEntityForName( "player" );
...
// JPW NERVE don't catch fire if invulnerable or same team in no FF
		if ( body->client ) {
			...
			if ( !( g_friendlyFire.integer ) && OnSameTeam( body,self->parent ) && ( body != self->parent ) ) {
				continue;
			}
		}
// jpw
...
// jpw
		...
		G_BurnMeGood( self, body ); //Flamethrower Fix
/*
        // add the new damage
        body->flameQuota += 5;
...
void G_RunFlamechunk( gentity_t *ent ) {
	...
	if ( level.time - ent->timestamp > 50 ) {
		//VectorCopy( ent->s.pos.trDelta, vel );	// removed cause duplicate of above
		//speed = VectorNormalize( vel );			// Project Bugfix
...
/*	if(self->methodOfDeath == MOD_LANDMINE)
    {
...
gentity_t *fire_grenade( gentity_t *self, vec3_t start, vec3_t dir, int grenadeWPID ) {
	...
	if ( g_gametype.integer >= GT_WOLF && ( grenadeWPID == WP_DYNAMITE || grenadeWPID == WP_DYNAMITE2 ) ) {
		...
		if ( self->client->ps.throwMine ) {
			if ( countmines( self->client->sess.sessionTeam ) == g_mines.integer ) {
				bolt->nextthink = level.time;
				trap_SendServerCommand( self - g_entities, va( "chat \"^7Your team is out of available mines!\n\"" ) );
	...
	bolt->r.ownerNum    = self->s.number;
	bolt->parent        = self;
	bolt->s.teamNum		= self->client->sess.sessionTeam; // cs: for _GetEntityTeam
...
//			bolt->splashDamage			= 100;
		...
		if ( g_shootableGrenade.integer ) {
			bolt->takedamage    = qtrue;
			bolt->health        = 1;
			bolt->die           = G_MissileDie;
		}
		if ( g_kickableNades.integer || g_shootableGrenade.integer ) {
...
// JPW NERVE
	case WP_SMOKE_GRENADE:
		bolt->classname             = "grenade";
		bolt->s.eFlags              = EF_BOUNCE_HALF | EF_BOUNCE;
		bolt->methodOfDeath         = MOD_AIRSTRIKE_CANISTER;
		bolt->splashMethodOfDeath   = MOD_AIRSTRIKE_CANISTER;

		if ( g_shootableAS.integer ) {
			bolt->takedamage    = qtrue;
			bolt->health        = 25;
			bolt->die           = G_MissileDie;
		}
		if ( g_kickableAS.integer || g_shootableAS.integer ) {
			bolt->r.contents            = CONTENTS_CORPSE;
			VectorSet( bolt->r.mins, -5, -5, 0 );
			VectorCopy( bolt->r.mins, bolt->r.absmin );
			VectorSet( bolt->r.maxs, 5, 5, 5 );
		...
		if ( ( g_smokeGrenades.integer ) && ( self->client->ps.stats[STAT_PLAYER_CLASS] == PC_LT ) && ( self->client->ps.throwSmoke ) ) {
...
// jpw
	...
	case WP_DYNAMITE2:
		...
		if ( self->client->ps.throwMine ) {
			...
			trap_SendServerCommand( self - g_entities, "cp \"Mine is set, but ^1NOT ^7armed!\"1" );
		} else {
			trap_SendServerCommand( self - g_entities, "cp \"Dynamite is set, but ^1NOT ^7armed!\"1" );
		}
		// differentiate non-armed dynamite with non-pulsing dlight

//			bolt->splashDamage			= 300;
		//Mine radius mine damage radius
		if ( self->client->ps.throwMine ) {
			bolt->s.teamNum = self->client->sess.sessionTeam + 4;
			//bolt->classname				= "grenade";	//martin old
			bolt->classname             = "dynamite";    //martin new
			//	bolt->damage				= 0;
			bolt->methodOfDeath         = MOD_LANDMINE;
			bolt->splashMethodOfDeath   = MOD_LANDMINE;
			bolt->splashRadius          = 400;
			bolt->s.eFlags              = ( EF_BOUNCE | EF_BOUNCE_HALF );   //martin old
			//bolt->s.eFlags				= EF_STICK;	//martin new (lol)
			bolt->die                   = G_MissileDie;
			self->client->ps.throwMine  = qfalse;
		} else {
			bolt->s.teamNum = self->client->sess.sessionTeam + 4;
...
// JPW NERVE -- blast radius proportional to damage
	if ( bolt->methodOfDeath != MOD_LANDMINE ) {
		if ( g_gametype.integer != GT_SINGLE_PLAYER ) {
...
void missile_think( gentity_t *ent ) {
	...
	target = NULL;
	targetlength = g_homingrocketdetects.integer;
	...
	for ( i = 0; i < level.maxclients; i++ ) {
		...
		if ( OnSameTeam( tent, ent->parent ) && ( g_deathmatch.integer != 2 ) ) {
			continue;
		}
		...
		if ( DotProduct( forward, tentdir ) < g_homingRocketsSens.integer ) {
			continue;
		}
	...
	VectorMA( forward, .05, targetdir, targetdir );
	VectorNormalize( targetdir );
	VectorScale( targetdir, g_rocketSpeed.integer, ent->s.pos.trDelta );
}
...
gentity_t *fire_rocket( gentity_t *self, vec3_t start, vec3_t dir ) {
	gentity_t   *bolt;
	int trType = TR_LINEAR;
	VectorNormalize( dir );

	bolt = G_Spawn();
	bolt->classname = "rocket";

	if ( g_homingRockets.integer == 0 ) {
		bolt->nextthink = level.time + 20000; // push it out a little
		bolt->think = G_ExplodeMissile;
	} else if ( g_homingRockets.integer )    { //Homing rockets :)
...
//	bolt->clipmask = MASK_SHOT;
	...
	if ( g_shootableRocket.integer ) {
		bolt->takedamage = qtrue;
		bolt->health = 10;
		bolt->r.contents         = CONTENTS_CORPSE;
		VectorSet( bolt->r.mins, -5, -5, 0 );
		VectorCopy( bolt->r.mins, bolt->r.absmin );
		VectorSet( bolt->r.maxs, 5, 5, 20 );
		VectorCopy( bolt->r.maxs, bolt->r.absmax );
		bolt->die           = G_MissileDie;
	}

	if ( g_panzerArc.integer == 1 ) {
		trType = TR_GRAVITY_LOW;
	} else if ( g_panzerArc.integer == 2 ) {
		trType = TR_GRAVITY;
	} else if ( g_panzerArc.integer == 3 ) { // Martin - eh its something new
		trType = TR_GRAVITY_FLOAT;
	}
//	else if (g_panzerArc.integer == 4)		// S4NDM4NN - expermental lol
