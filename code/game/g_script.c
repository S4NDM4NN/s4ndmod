...
void G_Script_ScriptLoad( void ) {
	...
	//SinglePlayer - Agent, Lets let all the noobs have working scripts...since we cant get them to use allowdownload,
	//				give them the single player maps (lol fuckers)
	if ( !Q_stricmp( mapname.string, "assault" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_assault.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "baseout" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_baseout.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "boss2" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_boss2.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "boss1" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_boss1.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "baseout" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_baseout.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "castle" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_castle.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "chateau" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_chateau.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "church" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_church.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "crypt1" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_crypt1.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "crypt2" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_crypt2.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "dam" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_dam.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "dark" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_dark.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "dig" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_dig.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "end" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_end.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "escape1" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_escape1.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "escape2" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_escape2.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "factory" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_factory.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "forest" ) ) {
		Q_strncpyz( filename, "maps/sniper_forest.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "norway" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_norway.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "rocket" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_rocket.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "sfm" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_sfm.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "swf" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_swf.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "trainyard" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_trainyard.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "tram" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_tram.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "village1" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_village1.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "village2" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_village2.script", sizeof( filename ) );
	}
	if ( !Q_stricmp( mapname.string, "xlabs" ) ) {
		Q_strncpyz( filename, "maps/s4nd_SP_xlabs.script", sizeof( filename ) );
	}

//	1.4/misc
	if ( !Q_stricmp( mapname.string, "mp_chateau" ) ) {
		Q_strncpyz( filename, "maps/s4nd_chateau.script", sizeof( filename ) );
	}
	if ( ( !Q_stricmp( mapname.string, "mp_ice" ) ) && ( g_gametype.integer != 7 ) ) {
		Q_strncpyz( filename, "maps/s4nd_ice.script", sizeof( filename ) );
	}
	if ( ( !Q_stricmp( mapname.string, "mp_rocket" ) ) && ( g_gametype.integer != 7 ) ) {
		Q_strncpyz( filename, "maps/s4nd_rocket.script", sizeof( filename ) );
	}
	if ( ( !Q_stricmp( mapname.string, "mp_assault" ) ) && ( g_sniperWar.integer ) ) {
		Q_strncpyz( filename, "maps/s4nd_assault.script", sizeof( filename ) );
	}
	if ( ( !Q_stricmp( mapname.string, "mp_gta2" ) ) && ( g_sniperWar.integer ) ) {
		Q_strncpyz( filename, "maps/s4nd_gta2.script", sizeof( filename ) );
	}
	if ( ( !Q_stricmp( mapname.string, "mp_base" ) ) && ( g_sniperWar.integer ) ) {
		Q_strncpyz( filename, "maps/s4nd_base.script", sizeof( filename ) );
	}

	//	Agent	-	Call custom cfg if we are doin a dm (S4ND_DM_%s.cfg ;)
	//	Not for sp maps though!
	if ( ( g_deathmatch.integer ) || ( g_panzerWar.integer ) || ( g_SpamWar.integer ) ) {
		if ( ( !Q_stricmp( mapname.string, "assault" ) ) || ( !Q_stricmp( mapname.string, "baseout" ) ) || ( !Q_stricmp( mapname.string, "boss1" ) ) || ( !Q_stricmp( mapname.string, "boss2" ) )
			 || ( !Q_stricmp( mapname.string, "castle" ) ) || ( !Q_stricmp( mapname.string, "chateau" ) ) || ( !Q_stricmp( mapname.string, "church" ) ) || ( !Q_stricmp( mapname.string, "crypt1" ) )
			 || ( !Q_stricmp( mapname.string, "crypt2" ) ) || ( !Q_stricmp( mapname.string, "dam" ) ) || ( !Q_stricmp( mapname.string, "dig" ) ) || ( !Q_stricmp( mapname.string, "end" ) )
			 || ( !Q_stricmp( mapname.string, "escape1" ) ) || ( !Q_stricmp( mapname.string, "escape2" ) ) || ( !Q_stricmp( mapname.string, "factory" ) ) || ( !Q_stricmp( mapname.string, "forest" ) )
			 || ( !Q_stricmp( mapname.string, "norway" ) ) || ( !Q_stricmp( mapname.string, "rocket" ) ) || ( !Q_stricmp( mapname.string, "sfm" ) ) || ( !Q_stricmp( mapname.string, "swf" ) )
			 || ( !Q_stricmp( mapname.string, "trainyard" ) ) || ( !Q_stricmp( mapname.string, "tram" ) ) || ( !Q_stricmp( mapname.string, "village1" ) ) || ( !Q_stricmp( mapname.string, "village2" ) )
			 || ( !Q_stricmp( mapname.string, "xlabs" ) ) ) {
		} else {
			Q_strncpyz( filename, va( "maps/s4nd_dm_%s.script",mapname.string ), sizeof( filename ) );
		}
	}
