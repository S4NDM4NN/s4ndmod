...
#define PANZER_CLIP 1           //DEFAULT 1
#define PANZER_HEAT 0           //DEFAULT 0
#define PANZER_COOL 0           //DEFAULT 0
#define PANZER_RATE 2000        //DEFAULT 2000
...
int weapBanksMultiPlayer[MAX_WEAP_BANKS_MP][MAX_WEAPS_IN_BANK_MP] = {
	{0,                     0,                      0,          0,          0,          0,              0,          0,      0   },  // empty bank '0'
	{WP_KNIFE,              0,                      0,          0,          0,          0,              0,          0,      0   },
	{WP_LUGER,              WP_COLT,                0,          0,          0,          0,              0,          0,      0   },
	{WP_MP40,               WP_THOMPSON,            WP_STEN,    WP_MAUSER,  WP_SHOTGUN, WP_PANZERFAUST, WP_VENOM,   WP_FLAMETHROWER,WP_FG42    },
	{WP_GRENADE_LAUNCHER,   WP_GRENADE_PINEAPPLE,   0,          0,          0,          0,              0,          0,      0   },
	{WP_MEDIC_SYRINGE,      WP_PLIERS,              WP_SMOKE_GRENADE,       0,          0,              0,          0,          0,  0       },
	{WP_DYNAMITE,           WP_MEDKIT,              WP_AMMO,    0,          0,          0,              0,          0,  0           }
};
// jpw
...
ammotable_t ammoTable[] = {
	...
	{   MAX_AMMO_FG42,  1,      20,     2000,   DELAY_LOW,      100,            0,          0,          MOD_FG42,				3		},  //	WP_FG42					// 5
	{   8,              1,      8,      1000,   DELAY_THROW,    1600,           0,          0,          MOD_GRENADE_LAUNCHER,	1		},  //	WP_GRENADE_LAUNCHER		// 6
	{   5,              1,  PANZER_CLIP,1000,   750,            PANZER_RATE,    PANZER_HEAT,PANZER_COOL,MOD_PANZERFAUST,		4		},  //	WP_PANZERFAUST			// 7	// DHM - Nerve :: updated delay so prediction is correct
//	{	MAX_AMMO_VENOM,	1,		500,	3000,	750,			30,				5000,		200,		MOD_VENOM,				4		},	//	WP_VENOM				// -
	...
	{   8,              1,      8,      1000,   DELAY_THROW,    1600,           0,          0,          MOD_GRENADE_PINEAPPLE,	1		},  //	WP_GRENADE_PINEAPPLE	// 17
...
//	{	MAX_AMMO_VENOM,	10,		300,	3000,	1200,			1200,			0,			0,			MOD_VENOM_FULL,			4		},	//	WP_VENOM_FULL			// -
	...
	{   MAX_AMMO_FG42,  1,      20,     2000,   DELAY_LOW,      400,            0,          0,          MOD_FG42SCOPE,			4		},  //	WP_FG42SCOPE			// 23
...
// stubs for some "not-real" weapons (so they always return "yes, you have enough ammo for that gauntlet", etc.)
	{   5,              1,      5,      1000,   DELAY_SHOULDER, 1200,   0,      0,      0, /*mod_prox*/	1		},  //	WP_PROX					// 34
	...
	{   999,            0,      999,    0,      50,             0,      0,      0,      0,				1		},  //  WP_HELL_IF_I_KNOW
	{   24,             1,      6,      1500,   DELAY_LOW,      1000,   0,      0,      MOD_SHOTGUN,	4		},  //  WP_SHOTGUN
	// -NERVE - SMF
...
*/
	{
		"ammo_grenades",
		"sound/misc/am_pkup.wav",
		{ "models/powerups/ammo/amgren_bag.mdc",
		  0, 0, 0,    0 },
...
*/
	{
		"ammo_panzerfaust",
		"sound/misc/am_pkup.wav",
		{ "models/powerups/ammo/ampf.md3", //models/powerups/ammo/ampf.md3
		  0, 0, 0,    0 },
...
*/
	{
		"ammo_panzerfaust", // ammo_rockets
		"sound/misc/am_pkup.wav",
		{ "models/powerups/ammo/amrocket.md3", //was rocketam
		  0, 0, 0,    0 },
		...
		WP_PANZERFAUST,
		WP_PANZERFAUST,
		WP_ROCKET_LAUNCHER, //WP_PANZERFAUST
		"",                     // precache
		"",                     // sounds
		{10,5,4,3,3}
	},
...
*/
	{
		...
		"sound/pickup/keys/binocs.wav",
		{
			...
			//"models/multiplayer/binocs/binocs.md3",	//Binocular Change - Agent
			0, 0, 0
			,   0
		},
		"icons/key6",           // icon
...
*/
	...
	*/
	...
	{
		"weapon_shotgun",   //----(SA)	modified
		"sound/misc/w_pkup.wav",
		{   "models/weapons2/winchester/winchester_3rd.md3",
			"models/weapons2/winchester/v_winchester.md3",
			"models/weapons2/winchester/winchester_pickup.md3",
			0, 0},

		"icons/iconw_shotgun_1",    // icon
		"icons/ammo5",              // ammo icon
		"Shotgun",              // pickup		//----(SA)	modified
		24,
		IT_WEAPON,
		WP_SHOTGUN, // this weap
		WP_SHOTGUN,     // shares ammo w/
		WP_SHOTGUN,     // shares clip w/
		"",                         // precache
		"",                         // sounds
		{0,0,0,0}
	},
...
qboolean    BG_CanItemBeGrabbed( const entityState_t *ent, const playerState_t *ps ) {
	...
	int deathmatch = g_deathmatch.integer;
	int packingPistols = g_packingPistols.integer;
	#elif defined ( CGAMEDLL )
	extern vmCvar_t cg_unlockWeapons;
	extern vmCvar_t cg_deathmatch;
	extern vmCvar_t cg_packingPistols;
	int unlockWeapons = cg_unlockWeapons.integer;
	int deathmatch = cg_deathmatch.integer;
	int packingPistols = cg_packingPistols.integer;
	#else
	int unlockWeapons = 0;
	int deathmatch = 0;
	int packingPistols = 0;
	#endif
	...
	case IT_WEAPON:
		...
		if ( deathmatch == 2 ) {
			//This is the SMG code here
			if ( ( item->giTag == WP_MP40 ) || ( item->giTag == WP_THOMPSON ) || ( item->giTag == WP_STEN ) ) {
				if ( item->giTag == WP_MP40 ) { //they run over an MP40
				...
				} else if ( item->giTag == WP_THOMPSON )     { //they run over a thomspon
					if ( COM_BitCheck( ps->weapons, WP_MP40 ) || COM_BitCheck( ps->weapons, WP_STEN ) ) { //but have mp40 or sten so just return false
						return qfalse;
					}
					...
					if ( COM_BitCheck( ps->weapons, WP_MP40 ) || COM_BitCheck( ps->weapons, WP_THOMPSON ) ) { //but have mp40 or Thompson so just return false
						return qfalse;
					}
				...
				return qtrue;    //let them have it
			}

			//This will be for pistol picking up(POSSIBLE CHANGE: Make so they can carry both?)
			if ( ( item->giTag == WP_COLT ) || ( item->giTag == WP_LUGER ) ) {

				if ( packingPistols ) { //let them add the pistol to their inventory!
					return qtrue;
				}

				if ( item->giTag == WP_COLT ) { //Look there we found us a colt
					if ( COM_BitCheck( ps->weapons, WP_LUGER ) ) { //But oh shit you have a luger so u cant have it
						return qfalse;
					}
					if ( ps->ammo[BG_FindAmmoForWeapon( WP_COLT )] >= ammoTable[WP_COLT].maxclip * 4 ) {
						return qfalse;
					}

				} else if ( item->giTag == WP_LUGER )     { //Now look what a nice luger on the ground
					if ( COM_BitCheck( ps->weapons, WP_COLT ) ) { //But DAMNIT you have a colt you need to lose 1st
						return qfalse;
					}
					if ( ps->ammo[BG_FindAmmoForWeapon( WP_LUGER )] >= ammoTable[WP_LUGER].maxclip * 4 ) {
						return qfalse;
					}
				}
				return qtrue;    //If they dont have one let 'em have it

			}
			//Ok so got pistol and got smg now what oh yes heavy weapons
			if ( ( item->giTag == WP_PANZERFAUST ) || ( item->giTag == WP_VENOM ) || ( item->giTag == WP_FLAMETHROWER ) || ( item->giTag == WP_MAUSER ) ) {
				if ( item->giTag == WP_PANZERFAUST ) { //oh boy its a boom stick
					if ( COM_BitCheck( ps->weapons, WP_VENOM ) || COM_BitCheck( ps->weapons, WP_FLAMETHROWER ) || COM_BitCheck( ps->weapons, WP_MAUSER ) ) { //aww to bad u have another heavy weapon
						return qfalse;
					}
					if ( ps->ammo[BG_FindAmmoForWeapon( WP_PANZERFAUST )] >= ammoTable[WP_PANZERFAUST].maxclip * 3 ) {
						return qfalse;
					}

				} else if ( item->giTag == WP_VENOM )     { //oh look at that venom wouldnt you like to have it...
					if ( COM_BitCheck( ps->weapons, WP_PANZERFAUST ) || COM_BitCheck( ps->weapons, WP_FLAMETHROWER ) || COM_BitCheck( ps->weapons, WP_MAUSER ) ) { //... haha noob lose your other heavy weapon
						return qfalse;
					}
					if ( ps->ammo[BG_FindAmmoForWeapon( WP_VENOM )] >= ammoTable[WP_VENOM].maxclip * 3 ) {
						return qfalse;
					}

				} else if ( item->giTag == WP_FLAMETHROWER )     { //oh boy oh boy its a flamer you will make people cry "NOOB" now
					if ( COM_BitCheck( ps->weapons, WP_PANZERFAUST ) || COM_BitCheck( ps->weapons, WP_VENOM ) || COM_BitCheck( ps->weapons, WP_MAUSER ) ) { //Nvm you have a heavy weapon
						return qfalse;
					}
					if ( ps->ammo[BG_FindAmmoForWeapon( WP_FLAMETHROWER )] >= ammoTable[WP_FLAMETHROWER].maxammo ) {
						return qfalse;
					}
				} else if ( item->giTag == WP_MAUSER )     {
					if ( COM_BitCheck( ps->weapons, WP_PANZERFAUST ) || COM_BitCheck( ps->weapons, WP_VENOM ) || COM_BitCheck( ps->weapons, WP_FLAMETHROWER ) ) {
						return qfalse;
					}
					if ( ps->ammo[BG_FindAmmoForWeapon( WP_MAUSER )] >= ammoTable[WP_MAUSER].maxclip * 3 ) {
						return qfalse;
					}
				}


				return qtrue;    //YOU CAN HAVE IT!!!! (wow im bored)
			}

			//So it wasnt heavy it wasnt smg and it wasnt pistol what was it hmmm... i guess let them have it
			//its probably just gonna be nades anyway
			return qtrue;

			//Ok lets see this crap compile :D
			//End S4NDM4NN

		} else {

			if ( packingPistols && ( item->giTag == WP_COLT || item->giTag == WP_LUGER ) ) { //let them add the pistol to their inventory!

				if ( ps->ammo[BG_FindAmmoForWeapon( item->giTag )] >= ammoTable[item->giTag].maxclip * 4 ) {
					return qfalse;
				}
			...
			if ( ps->stats[STAT_PLAYER_CLASS] == PC_ENGINEER ) {
				if ( !COM_BitCheck( ps->weapons, item->giTag ) ) {
					...
					if ( unlockWeapons == 0 ) { //if this cvar is disabled, then behave like normal
						return qfalse;
					}
					//if it's 1, meds and engs can pickup smg's
					else if ( ( unlockWeapons == 1 ) && ( ( item->giTag != WP_MP40 ) && ( item->giTag != WP_THOMPSON ) && ( item->giTag != WP_STEN ) && ( item->giTag != WP_FG42 ) && ( item->giTag != WP_FG42SCOPE ) && ( item->giTag != WP_SHOTGUN ) ) ) {
						return qfalse;
					}
					//if it's 2+, they can pickup anything - don't return here, because we need to go through later loop
				} else
				{
					return qtrue; //if they already have the gun, they can pick up another (for ammo), so no need to edit this
				}
			}
			...
			if ( ps->stats[STAT_PLAYER_CLASS] == PC_LT ) {
				if ( ( item->giTag != WP_MP40 ) && ( item->giTag != WP_THOMPSON ) && ( item->giTag != WP_STEN ) && ( item->giTag != WP_SHOTGUN ) ) {

					if ( unlockWeapons < 2 ) {

						return qfalse; //this will be part of the 'if' statement if it is compiled in
					}
				}
			}
		}
