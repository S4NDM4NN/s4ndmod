...
#define GAMEVERSION "S4NDMoD 2.5.0"
#define MODURL "http://www.s4ndmod.com"
...
struct gentity_s {
	...
	int thrownSmoke;
	int thrownMine;
	int TimesReclaimed;                     //for Retake Limit
	int drunk;
	int nextDrunkTime;
	qboolean isbanned;
	int flagteam;
	int count3;        //s4ndm4nn used by auto mg42
	int lastAutoTapCheckTime;

	qboolean isMine;

	// Indoor arty  / indoor artillery...props to dutchmeat
	qboolean dmginloop;
	gentity_t   *dmgparent;

	// Anticheat
	float zbc_angles[2][2];         // Martin - Screw it, give us lots of decimal points
	int zbc_tog;
	int zbc_jitter;
...
typedef struct {
	team_t sessionTeam;
	//int			teamflag;			//agent keep track of who has the s4ndflag
	...
	//int         guidentered;
	//int         guid;	//Martin - Removed
	//qboolean	autoReload;	// OLD...just get it from userinfochanged which is called on new round
	// dont force sess data to hold redundent info
	...
	// L0
	int incognito;
	unsigned int uci;  //L0 - mcwf's GeoIP

} clientSession_t;

//S4NDM4NN
typedef struct {
//This will be used for the stats
	int kills;
...
//S4NDM4NN for the stats
	...
	int hitleg;
	int dmgGiven;
	int dmgTeam;                //	This is damage to TEAMMATES
	int dmgreceived;
	int nextSpecLockCheck;
	int spawnNum;
	qboolean gibbed;
	int LastNameChangeTime;
	char endroundstats[1024];
	char ownendroundstats[1024];
	char location[1000];


	qboolean privateclient;
	qboolean latejoiner;
...
//End
	...
	int clientmg42;        //the entity num of the mg42 owned by this client

	//int			lastAngle;	// Martin - centerview test
	vec3_t lastAngle;
	qboolean isTapped;
	int lastTapMessageTime;
	int spawnCyclesSkipped;
	qboolean autoReload;
	qboolean hitsounds;
	qboolean multikill;
	int numBinocs;
	int lastTeamkilledPlayer;
	int spawnTimerStartTime;
	qboolean spawnTimerActive;
	int lastSpawnTimerCheck;
	int lastShovedTime;             //When was I shoved
	//int			lastShovedClient;	//Who shoved me
	gentity_t   *lastShovedClient;
...
// L0
	int deathSpree; // Deaths for DeathSpree count- since deaths are used elsewhere it wouldn't be wise to reset them on kill and risk breaking /stats.. so new one =)
	int ready;      // New Ready/NotReady system...
	qboolean stuck;     // used for pausing game
	// L0 - end

#ifdef GLOBALSTATS //client persistant varibles for global stats
	int statNum;        //stat num is set by stat system each round.
	int lastWeapon;     //will be used to detect weapon switches
	int usingMg42;      //to detect if we switched from MG42
	int usingBinoc;     //to detect if we switched from Binocs
#endif

} clientPersistant_t;

//ANTILAG - client origin trails
...
typedef struct {
	...
	int frameStartTime;                 //ANTILAG - actual time frame started
...
// jpw
	...
	//S4NDM4NN-for msgs
	int LastMsgTime;
	int LastBinocWarTime;
	int LastCVARcheck;
	int LastFraglimitScore;
	int LastMapRotationCheck;        //auto rotation
	int LastDemoHeartbeat;
	int LastReadyMsg;
	int readymsgnum;
	qboolean allready;
	int LastDMpoint;        //for the making of the DM points
	int DMspawnpoint;        //for the rotation of spawnpoints
	int RedSniperSpawnPoint;
	int BlueSniperSpawnPoint;
	int RedDefaultSpawn;
	int BlueDefaultSpawn;
	int locationCheckTime;
	int LastGunSpawnCheck;
	int flagteam;
	int AxisLeft;               //Agent for maxlives
	int AlliesLeft;             //Agent for maxlives
	int LastAutoTapCheckTime;
	int axisAirstrikeCounter;
	int alliedAirstrikeCounter;         //
	int LastAirstrikeCheck;
	int axisMinesAvailable;
	int alliedMinesAvailable;
	int balanceTime;
	qboolean statsPrinted;
	int lastThreadCheck;
	//End
	...
	//All vs All dm
	int highkill;        //the amount of kills the leader has
	gentity_t   *highkiller; //this is the leader
	int endDeathmatch;        //this is to tell us if round is over so we can start "intermission"
	int nextDeathmatchendTime;

	int HAkillnum;
	char HAhighkillers[MAX_STRING_CHARS];
	int HAgibnum;
	char HAhighgibbers[MAX_STRING_CHARS];
	int HAdeathnum;
	char HAhighdeaths[MAX_STRING_CHARS];
	int HAgoombanum;
	char HAhighgoombaers[MAX_STRING_CHARS];
	int Hapoisonnum;
	char Hahighpoisoners[MAX_STRING_CHARS];
	int HArevivenum;
	char HAhighrevivers[MAX_STRING_CHARS];
	int HAammonum;
	char HAhighammogivers[MAX_STRING_CHARS];
	int HAmednum;
	char HAhighmedgivers[MAX_STRING_CHARS];
	int HAHshotnum;
	char HAheadhunters[MAX_STRING_CHARS];
	int HAtknum;
	char HAtkers[MAX_STRING_CHARS];
	char HAaccuracy[MAX_STRING_CHARS];
	char HAhighestACC[MAX_STRING_CHARS];
	char HAkr[MAX_STRING_CHARS];
	char HAhighestKR[MAX_STRING_CHARS];
	qboolean useACC;
	qboolean useKR;
	qboolean HAdone;
	qboolean HAstartMatch;
	int HAlastPrintTime;
	int HAprintnum;
	int lastCleanClientCheck;
	int axismg42s;
	int alliedmg42s;
	int maxautomg42s;
	FILE    *   S4NDLog;
	//FILE	*	AdminLog; removed this untill we are actually going to use it?
	...
	// L0
	int firstbloodTeam, firstBloodTrue;     // FirstBlood
	int countEnd, aEnd;     // Ready system - count down/ready All...
	int paused;             // Match is paused
	int timeDelta;          // Pause
	int timeCurrent;        // Pause
	int pendingUnpause, killCPmsg;     // Pause Limit / cp info..
	int CNyes, CNstart, CNpush;     // Countdown for pause
	// L0 - end
...
void G_ExcludeClientFromCommand( int num, char *cmd );
...
//#ifdef OMNIBOT //S4NDM4NN took these out of omni since they are being used by s4ndmod
...
void G_BurnMeGood( gentity_t *self, gentity_t *body );   //Flamethrower Fix
...
extern vmCvar_t g_debugSpawn;       //Martin
...
extern vmCvar_t g_banmsg;
extern vmCvar_t g_tempbanmsg;
